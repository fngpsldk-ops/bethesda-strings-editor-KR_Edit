...
버전별로 정리되어 있습니다 (`v1.0.1_KR`처럼). 아래 "버그 수정"부터
"프롬프트 튜닝 (PROMPT_VERSION 1 → 2)"까지의 모든 내용은 최초 릴리스인
**v1.0.0_KR** (2026-07-02, 앱 내 "새 소식" 패널의 첫 항목)에 해당합니다.

---

## v1.0.0_KR — 최초 릴리스: 클라우드 AI 백엔드, 프롬프트 에디터, 한글화
...
### 2차 세션: OpenAI 호환 백엔드 GUI 통합 + 한글 UI + 안정화 (v1.0.0_KR에 포함)
...
## v1.0.1_KR — Translation Memory 조회 정확도 및 성능

실제 모드 파일(rbt_RealFuel_RE, rbt_RealO2 등)을 반복 번역하며 드러난 TM
오매칭 문제를 재현 → 근본 원인 진단 → 실제 데이터로 검증하는 방식으로 진행.
...
## v1.0.2_KR — esp_handler 필드 매핑 버그 및 AI 환각 방지
...
## v1.0.3_KR — 캐시 신뢰성: 수동 교정 반영 및 재번역 무력화 방지
...
## v1.0.4_KR — API 요청 제한 대응 및 동시성 안정화

`gui/openai_compat_worker.py`:
- **429 재시도 로직 부재**: TM 조회 최적화(v1.0.1)로 요청 속도가 빨라지면서,
  Gemini 무료 티어의 분당 10회 제한을 순식간에 초과 → 대량 429 발생, 재시도
  없이 즉시 실패 처리되던 문제. `Retry-After` 헤더 존중 + 지수 백오프(최대
  5회) 추가.
...
## v1.0.5_KR — 테이블 정렬 기능 및 번역 중 데이터 무결성
...
## v1.0.6_KR — 신규 워크플로 기능: 강제 재번역, 일괄 적용, TM 뷰어, 출처 색상
...
## v1.0.7_KR — UI/UX 개선 및 포터블 배포 지원
...
## v1.1.1_KR — WEAP 등 반복 필드 레코드에서 번역이 한 칸씩 밀리는 버그 수정

`bethesda_strings/esp_handler.py`:
- **추출(`_parse_record`)과 재작성(`_patch_fields`)의 occurrence 판정 기준이
  서로 달랐던 문제**: 같은 필드 서명이 한 레코드 안에 여러 번 나오는 경우
  (예: WEAP의 `FULL`이 무기 이름 1개 + 개조 등급 이름 10개로 총 11번 등장),
  추출 쪽은 빈 값·경로처럼 보이는 값을 건너뛰는데 재작성 쪽은 그 판단을
  하지 않아 순번이 어긋났음. 두 함수가 공유하는 `_field_translatable_text()`
  로 판정 로직을 통합.
- **`save()`의 occurrence 카운터가 "원문과 동일하게 유지된(번역 안 된)"
  항목에서는 증가하지 않던 문제**: 브랜드/무기 이름처럼 번역해도 원문과
  같은 문자열이 레코드 안에 하나라도 있으면, 그 뒤의 같은-필드 번역이
  전부 한 칸씩 밀려 들어가는 심각한 버그였음. 실제 사례: 무기 "ARX-15"의
  이름 필드가 통째로 사라지고, 그다음 순번인 "Fallback"의 번역("폴백")이
  이름 자리에 잘못 들어감. 카운터가 번역 여부와 무관하게 모든 occurrence
  마다 증가하도록 수정.

`gui/translation_memory.py`:
- **`__bool__`/`__len__`이 `_by_id`만 확인하던 문제**: TMX로 불러온 번역
  메모리는 `_by_id`를 안 채우고 `_by_src`만 채우는데, 이 판정 하나가 TM
  뷰어 표시뿐 아니라 실제 번역 시 TM 조회 여부(`if self.translation_memory:`)
  까지 결정하고 있어서, **TMX로 불러온 TM이 뷰어에 안 보이는 것은 물론
  실제 번역에도 전혀 반영되지 않던** 문제. `_by_src`도 함께 확인하도록 수정.

---

## v1.1.2_KR — 버전 표기 동기화 및 새 소식 패널 자동화

`main.py` / `gui/app_version.py`(신규) / `gui/main_window.py`:
- **앱이 항상 "0.2.3"으로 표시되던 버그**: 엔트리포인트(`main.py`)가
  `setApplicationVersion("0.2.3")`으로 버전을 하드코딩하고 있어서, CI가
  태그로 갱신하는 `_version.py`와 완전히 단절돼 있었음. 업데이트 확인
  팝업의 "현재 버전"도 이 값을 읽으므로 항상 틀린 비교를 하고 있었음.
  단일 해석기(`resolve_app_version`)로 통합: CI가 심은 `_version.py` →
  없으면 CHANGES.md 최신 섹션 → 없으면 "dev" 순서로 버전 결정. 로컬
  빌드도 이제 올바른 버전을 표시함.
- 아무 데서도 참조되지 않던 중복 엔트리포인트 `gui/main.py`(별도의
  잘못된 버전 "1.0.0_KR"이 또 하드코딩돼 있었음) 삭제.
- **타이틀바에 현재 버전 표시**: 시작 시와 파일 열림/세션 전환 후 모두
  "Bethesda Strings AI Translator v1.1.2_KR" 형태로 유지.
- **새 소식 패널이 CHANGES.md를 자동 반영**: 릴리스마다 손으로 고쳐야
  했던(그래서 실제로 방치되던) 정적 HTML 대신, exe 옆에 함께 배포되는
  CHANGES.md를 파싱해서 최신 항목부터 표시. 현재 설치된 버전에는
  "(installed)" 배지가 붙음. About 대화상자 버전 표기도 같은 소스로 통일.
