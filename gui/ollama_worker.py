...
DEFAULT_PERSONA = (
    "당신은 전문 베데스다 스타필드 게임 로컬라이제이션 번역가입니다."
)
...
_TARGET_STYLE: dict[str, str] = {
    ...
    "ko": (
        "스타필드의 SF 톤에 맞는 한국어를 쓰세요. "
        "UI 라벨, 시스템 메시지, NPC 대사는 원문이 명백히 캐주얼하지 않은 이상 "
        "격식체(합쇼체/해요체)를 쓰세요. "
        "SF 고유명사는 한글 음역으로 표기하세요 "
        "(예: New Atlantis → 뉴 애틀랜티스, Sarah → 사라). "
        "기술적 안내문은 현재형을 씁니다(예: 시스템 정상). "
        "자연스러운 한국어 대체어가 있으면 불필요한 영어 외래어를 피하세요. "
        "영문 약어/숫자 조합(O2, CO2, H3, SFSE 등)에 조사를 붙일 때는 "
        "실제 한글 발음 기준으로 붙이세요 — 로마자 철자가 아니라 소리 나는 대로 "
        "판단합니다. 예: O2는 '오투'로 끝나 모음 받침이라 '는/를/가'를 쓰고 "
        "('O2는', 'O2를', 'O2가'), CO2는 '씨오투'로 끝나 마찬가지로 '는/를/가'를 씁니다. "
        "H3처럼 자음('쓰리')으로 끝나면 '은/을/이'를 씁니다. 확신이 안 서면 "
        "숫자를 소리 내어 읽어보고 그 마지막 음절의 받침 유무로 판단하세요."
    ),
    "pl": (
